...
class ScaleControlView {
  ...
  onHandlePointerDown(event) {
    if (
      event.target === this.refs.handleLeft ||
      ...
      this.state.handleRightPct =
        (this.containerWidth - this.handleRightX) / this.containerWidth;
    }
...
class BoardView {
    ...
    return $.div(
      {
        ...
        style: `width: ${Math.round(w)}px;
                ...
                ${this.dragging ? "opacity: 0.8; filter: saturate(120%);" : ""}
        ...
        this.mini
          ? $.div({
              style: `position: absolute;
                  ...
                  padding: ${this.mini ? 0 : kind === "board" ? "5px" : "0"};
                  box-sizing: border-box;
                  ...
                  ? $.div(
                      {
                        ...
                        font-size: 10px;
                        font-weight: 100;
                        color: ${this.active ? "rgba(0, 0, 0, 0.8)" : "#999"}`,
                      },
                      [
                        $.div(
                          { class: "board__number", style: "font-weight: 700" },
                          `${this.board.shot}`,
                        ),
                        ...
                        $.div(
                          { class: "board__duration" },
                          `${msToTime(boardModel.boardDuration(this.scene, this.board))}`,
                        ),
                      ],
                    )
                  : null,
  ...
  async writeAfterUpdate() {
    if (this.kind === "audio" && !this.mini) {
      ...
      let data = this.getAudioBufferByFilename(
        this.board.audio.filename,
      ).getChannelData(0);
...
class TimelineView {
  ...
  render() {
    ...
    for (let board of this.scene.boards) {
      ...
      if (board.audio) {
        ...
        if (buffer) {
          ...
          lanes[currLane].endInMsecs =
            timelinePosInMsecs + audioDurationInMsecs;
    ...
    let audioLanes = lanes.map((lane) =>
      etch.dom(
        LaneView,
        { kind: "audio", mini: this.mini },
        lane.boards.map((board) =>
          etch.dom(BoardView, {
            pixelsPerMsec: this.pixelsPerMsec,
            scale: this.scale,
            board: board,
    ...
    if (this.state.draggableBoardView) {
      let movementX =
        this.state.draggableOffsetInPx / (this.pixelsPerMsec * this.scale);
      ...
      let times = this.scene.boards
        .map((b) => b.time)
        .concat(sceneModel.sceneDuration(this.scene));
      this.state.insertPointInMsecs = closest(times, goalTime);

      let caretLeft =
        this.state.insertPointInMsecs * this.pixelsPerMsec * this.scale;
    ...
    let cursor = this.state.draggableBoardView ? "-webkit-grabbing" : null;

    let boardLane = etch.dom(
      LaneView,
      { kind: "board", mini: this.mini },
      boardsViews,
    );
    ...
    return $.div({ style: "position: relative;" }, [
      $.div(
        ...
        $.div(
          ...
          [boardLane, !this.mini ? audioLanes : null],
        ),
      ),
      caretView,
    ]);
  ...
  async onBoardPointerDown(event, boardView) {
    ...
    if (event.target === boardView.refs.handle) {
      ...
      this.state.resizableBoardOriginalDuration = boardModel.boardDuration(
        this.scene,
        boardView.board,
      );
  ...
  async onDocumentPointerMove(event) {
    ...
    if (this.state.resizableBoardView) {
      let index = this.scene.boards.indexOf(
        this.state.resizableBoardView.board,
      );
      let movementX =
        this.state.resizableOffsetInPx / (this.pixelsPerMsec * this.scale);
  ...
  async completeDragOrResize(event) {
    ...
    if (this.state.draggableBoardView) {
      if (this.state.insertPointInMsecs != null) {
        if (
          this.state.insertPointInMsecs === sceneModel.sceneDuration(this.scene)
        ) {
          // move to very end of scene
          selections = new Set([
            this.scene.boards.indexOf(this.state.draggableBoardView.board),
          ]);
          position = this.scene.boards.length;
        } else {
          let boardToInsertBefore = this.scene.boards.find(
            (board) => board.time === this.state.insertPointInMsecs,
          );
  ...
  async writeAfterUpdate() {
    ...
    */
    ...
    let entireWidth =
      this.sceneDurationInMsecs * this.pixelsPerMsec * this.scale;
...
class SceneTimelineView {
  constructor(props, children) {
    ...
    this.onSetCurrentBoardIndex = props.onSetCurrentBoardIndex;
    this.onModifyBoardDurationByIndex = props.onModifyBoardDurationByIndex;
    this.onSeekToTime = props.onSeekToTime;
  ...
  render() {
    ...
    let currTime = msToTime(
      Math.floor(this.scene.boards[this.currentBoardIndex].time / 1000) * 1000,
    );
  ...
  update(props, children) {
    ...
    if (props.pixelsPerMsec != null) this.pixelsPerMsec = props.pixelsPerMsec;
    if (props.mini != null) this.mini = props.mini;
    if (props.onSeekToTime != null) this.onSeekToTime = props.onSeekToTime;
    ...
    if (props.currentBoardIndex != null) {
      if (this.currentBoardIndex !== props.currentBoardIndex) {
        if (this.scale > 1) {
          ...
          let dur =
            boardModel.boardDuration(this.scene, board) / sceneDurationInMsecs;
          let end = start + dur;
    ...
    if (props.position != null && props.scale != null) {
